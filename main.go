...
// say announces the remaining seconds using Linux text-to-speech.
// Tries spd-say first (speech-dispatcher), falls back to espeak/espeak-ng.
func say(d time.Duration) {
	if d.Seconds() <= 10 {
		text := fmt.Sprintf("%v", d.Seconds())

		// Try spd-say (speech-dispatcher, common on most Linux distros)
		...
		// Silent fallback — no TTS available
	}
}
